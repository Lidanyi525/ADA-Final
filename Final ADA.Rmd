...
mutate(participate_pe = recode_factor(participate_pe, "Don't know" = NA_character_)) %>%
# Q 35 School nurse obtains materials and medical supplies (including the emergency supply kit) for performing diabetes care tasks from the parents/guardians, and designate a storage location for daily and emergency use.
...
, "Don't know" = NA_character_))%>%
# Mental Health/ Right
# Q 55 Student Stress Levels
mutate(effect_stress
 = recode_factor(effect_stress
, "Don't know" = NA_character_))%>%
...
# Q 51 Student Glycemic Control
mutate(effect_glycemic
 = recode_factor(effect_glycemic
, "Don't know" = NA_character_)) %>%
# Q 52 Student Physical Health
mutate(effect_physical
 = recode_factor(effect_physical
, "Don't know" = NA_character_)) %>%
# Q 53 Student Mental Health
mutate(effect_mental
 = recode_factor(effect_mental
, "Don't know" = NA_character_))
...
table(Survey_cleaned$f3Nurse)

```
```{r warning=FALSE}
Complete <- Survey_cleaned %>%
  drop_na()

MissingRate <-  1- nrow(Complete)/nrow(Survey_cleaned)
...
summary(Complete$effect_overall)
levels(Complete$effect_overall)

Complete$effect_overall <- relevel(Complete$effect_overall, ref = "Slightly effective")

mod <- multinom(effect_overall ~ f1TDP + f2Plan + f3Nurse + f4Communication + effect_stress + gender + race + effect_physical + effect_mental, data=Complete)
